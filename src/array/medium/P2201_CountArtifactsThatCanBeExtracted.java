 ...
 */
public class P2201_CountArtifactsThatCanBeExtracted {

    public int digArtifacts(int n, int[][] artifacts, int[][] dig) {
        // 记录单元格是否被挖掘
        boolean[] bare = new boolean[n * n];
        for (int[] d : dig) {
            bare[d[0] * n + d[1]] = true;
        }
        int count = 0;
        for (int[] artifact : artifacts) {
            // 是否裸露
            boolean flag = true;
            for (int r = artifact[0]; r <= artifact[2] && flag; r++) {
                for (int c = artifact[1]; c <= artifact[3] && flag; c++) {
                    flag = bare[r * n + c];
                }
            }
            // 范围内所有单元格都是将要被挖掘的
